        ...
        try {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    if (dirMatch && Number.parseInt(dirMatch[1], 10) === numericPlanId) {
                        ...
                        try {
                            ...
                            for (const planEntry of planDirEntries) {
                                if (planEntry.isFile()) {
                                    ...
                                    if (fileMatch && Number.parseInt(fileMatch[1], 10) === numericPlanId) {
...
function listAvailablePlans() {
    ...
    return plans.sort((a, b) => {
        const aId = Number.parseInt(a.match(/^(\d+)--/)[1], 10);
        const bId = Number.parseInt(b.match(/^(\d+)--/)[1], 10);
        return aId - bId;
    });
}
