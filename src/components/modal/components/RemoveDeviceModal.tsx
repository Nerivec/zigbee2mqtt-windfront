...
type DeviceRemovalButtonProps = {
    sourceIdx: number;
    device: Device;
    removeDevice(sourceIdx: number, ieee: string, force: boolean, block: boolean, keepConfig: boolean, clearCache: boolean): Promise<void>;
};
...
export const RemoveDeviceModal = NiceModal.create(({ sourceIdx, device, removeDevice }: DeviceRemovalButtonProps): JSX.Element => {
    ...
    const [removeParams, setRemoveParams] = useState({ block: false, force: false, keepConfig: false, clearCache: false });
    ...
    const onRemoveClick = useCallback(async () => {
        modal.remove();
        await removeDevice(sourceIdx, device.ieee_address, removeParams.force, removeParams.block, removeParams.keepConfig, removeParams.clearCache);
        ...
        >
            <div className="flex flex-col gap-2">
                ...
                <CheckboxField
                    ...
                    checked={removeParams.keepConfig}
                />
                <CheckboxField
                    label={t(($) => $.clear_cache)}
                    name="clearCache"
