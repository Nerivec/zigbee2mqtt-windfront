...
const AddDevicesToGroup = memo(({ sourceIdx, devices: allDevices, group }: AddDevicesToGroupProps) => {
    ...
    const availableDevices = useMemo(() => {
        return allDevices.filter(({ ieee_address }) => {
    ...
    const onAddDevicesToGroupClick = useCallback(async () => {
        await Promise.all(
            selectedDevices.map(async ({ device, endpoint }) => {
                if (endpoint === "") {
                    return;
                }
    ...
    const canAddDevicesToGroup = selectedDevices.length > 0 && selectedDevices.every(({ endpoint }) => !!endpoint);

    return (
        <>
            <h2 className="text-lg font-semibold">{t(($) => $.add_to_group_header)}</h2>
            <div className="mb-3 flex flex-col gap-3 grow">
                <DevicePicker value={devicePickerValue} devices={availableDevices} onChange={onDevicePick} />
