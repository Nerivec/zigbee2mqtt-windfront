...
export default function SyncRetryButton() {
    ...
    }, [writeState?.isConflict, writeState?.isTimedOut, onSync, onRetry]);

    // Determine if button should show
    const showButton = onSync || writeState?.isConflict || writeState?.isTimedOut || writeState?.isPending;
    ...
    const icon = writeState?.isConflict || writeState?.isTimedOut ? faRedo : faSync;
